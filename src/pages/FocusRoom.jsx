...
import { Play, Pause, Square, Settings2, Coffee, Globe, Timer } from "lucide-react";
...
export default function FocusRoom() {
  const [activeTab, setActiveTab] = useState("focus"); // "focus" | "worlds"
  ...
  // per-course task counts this session
  ...
  return (
    <div className="min-h-screen" style={{ background: activeTab === "worlds" ? "#08080f" : "linear-gradient(135deg, #fdf6ec 0%, #fef3e2 50%, #f0ebe8 100%)" }}>

      {/* ── Tab Bar ── */}
      <div className={`sticky top-0 z-30 flex items-center gap-1 px-6 py-3 border-b backdrop-blur-sm ${
        activeTab === "worlds"
        ...
        {activeTab === "worlds" ? (
          ...
          >
            <div className="max-w-6xl mx-auto space-y-6">
              ...
              {/* World status strip */}
              <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border border-stone-200/60 bg-white/60 backdrop-blur-sm">
                <span className="text-2xl">{archetype.emoji}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-bold text-stone-700 truncate">{archetype.name}</p>
                    <span className="text-xs text-stone-400 shrink-0 ml-2">{LEVEL_NAMES[worldLevel]}{worldLevel < 5 ? ` → ${nextLevel}` : ""}</span>
                  </div>
                ...
                <button
                  onClick={() => setActiveTab("worlds")}
                  className="shrink-0 text-xs text-stone-400 hover:text-primary underline"
                >
                  Switch world
                </button>
              </div>
